...
class Test_otherDataPage():
    ...
    @allure.story("选择期货或衍生产品, 投资目标选择'利息/股息收入', 查看提示")
    @pytest.mark.parametrize("reloadRoute", fix_routetitle, indirect=True)
    @pytest.mark.skipif(gm.get_value("derivative") == True and "futuresMargin" in gm.get_value("accountType"),
                        reason="跳过用例")
    ...
    @allure.story("选择期货或衍生产品, 投资目标选择'利息/股息收入', 查看提示")
    @pytest.mark.parametrize("reloadRoute", fix_routetitle, indirect=True)
    @pytest.mark.skipif("futuresMargin" in gm.get_value("accountType"),reason="跳过用例")
    ...
    @allure.story("选择期货或衍生产品, 投资目标选择'利息/股息收入', 查看提示")
    @pytest.mark.parametrize("reloadRoute", fix_routetitle, indirect=True)
    @pytest.mark.skipif(gm.get_value("derivative") == True,reason="跳过用例")
...
if __name__ == "__main__":
    pytest.main(["-s", "-v", "--pdb", "test_13_otherData.py::Test_otherDataPage", '--alluredir', '../report/xml_{time}'.format(time=datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))])
